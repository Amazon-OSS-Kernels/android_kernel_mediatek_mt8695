...
/*In order to dynamically on/off mm-pm in y-cable suspend&resume*/
static bool fg_mm_pm_state;
...
static int mtkfb_pan_display_impl(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	int ret = 0;
	struct mtk_disp_config *pConfig = NULL;
	struct disp_hw_common_info hw_info;

	pConfig = vmalloc(sizeof(struct mtk_disp_config));
	if (pConfig == NULL)
		return -1;
	memset(pConfig, 0, sizeof(struct mtk_disp_config));
	...
	pConfig->buffer_num = 1;
	pConfig->buffer_info[0].layer_id = 0;
	pConfig->buffer_info[0].layer_enable = 1;
	pConfig->buffer_info[0].src_base_addr = vaStart;
	pConfig->buffer_info[0].src_phy_addr = (void *)paStart;
	pConfig->buffer_info[0].src_fmt =
		(var->blue.offset == 0) ? DISP_HW_COLOR_FORMAT_BGRA8888 : DISP_HW_COLOR_FORMAT_RGBA8888;
	pConfig->buffer_info[0].type = DISP_LAYER_OSD;
	pConfig->buffer_info[0].src.x = 0;
	pConfig->buffer_info[0].src.y = 0;
	pConfig->buffer_info[0].src.width = var->xres;
	pConfig->buffer_info[0].src.height = var->yres;
	pConfig->buffer_info[0].tgt.x = 0;
	pConfig->buffer_info[0].tgt.y = 0;
	pConfig->buffer_info[0].tgt.width = hw_info.resolution->width;
	pConfig->buffer_info[0].tgt.height = hw_info.resolution->height;
	pConfig->buffer_info[0].alpha_en = 0;
	pConfig->buffer_info[0].ion_fd = -1;
	pConfig->buffer_info[0].acquire_fence_fd = -1;
	pConfig->buffer_info[0].res_mode = hw_info.resolution->res_mode;

	ret = disp_hw_mgr_config(pConfig);
...
static void mtkfb_shutdown(struct platform_device *pdev)
{
	...
	fbdev = dev_get_drvdata(&pdev->dev);
	mtkfb_free_resources(fbdev, fbdev->state);
	fbdev->state = MTKFB_DISABLED;
