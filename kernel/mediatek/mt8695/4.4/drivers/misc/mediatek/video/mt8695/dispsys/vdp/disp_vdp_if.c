			...
			if (layer_info->release_timeline_idx > buf->current_fence_index) {
				...
				if (layer_info->timeline != NULL) {
					...
					if (print_video_fence_history) {
						...
						DISP_LOG_N("fence%d time %lld, wait 0x%llx, pts %lld, timer %ld\n",
...
int disp_vdp_start(struct disp_hw_common_info *info, unsigned int layer_id)
{
	...
	*/
    ...
    /* notify hdr start enabled */
	hdr_layer_id = layer_id;
	vdp_drv->drv_call(DISP_CMD_VIDEO_PLAY_START,
		&hdr_layer_id);
	/* vdp_disable_active_zone(layer_info); */
...
int disp_vdp_config(struct mtk_disp_buffer *config, struct disp_hw_common_info *info)
{
	...
	*/
	...
	if (ret < 0) {
		if ((video_layer[config->layer_id].type == config->type) &&
			...
			DISP_LOG_D("buffer_layer_id: %d get mva fail ionfd:0x%x waitFence[0x%x]\n",
				config->layer_id,
				config->ion_fd,
	...
	*/
	...
	vdp_printf(VDP_AVSYNC_LOG, "vdo%d dolby %d hdr_type %d\n",
		buf_info->layer_id, buf_info->is_dolby, buf_info->hdr10_type);

	vdp_printf(VDP_AVSYNC_LOG,
		"vdo%d ion=%d hnd=0x%p pts=%lld rel=%d cur=%d dr=%d->%d 0x%X (0x%X 0x%X 0x%X 0x%X) release %d\n",
		buf_info->layer_id, buf_info->ion_fd, buf_info->ion_hnd, config->pts,
	...
	vdp_printf(VDP_AVSYNC_LOG, "vdo%d, src(%d %d %d %d %d) crop(%d %d %d %d %d) tgt(%d %d %d %d %d)\n",
		buf_info->layer_id,
		buf_info->src.x, buf_info->src.y, buf_info->src.width, buf_info->src.height, buf_info->src.pitch,
		buf_info->crop.x, buf_info->crop.y, buf_info->crop.width, buf_info->crop.height, buf_info->crop.pitch,
		buf_info->tgt.x, buf_info->tgt.y, buf_info->tgt.width, buf_info->tgt.height, buf_info->tgt.pitch);
