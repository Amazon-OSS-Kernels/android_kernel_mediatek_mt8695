...
static unsigned int port_num[MT8695_SMI_LARN_NR] = {2, 11, 0, 11, 7, 10, 13, 6, 2};
static inline int m4u_get_larbid(int kernel_port)
{
	unsigned int i, port_idx;

	i = kernel_port >> 5;
	port_idx = kernel_port & 0x1f;
	if (i < MT8695_SMI_LARN_NR && i != 2)
		if (port_idx < port_num[i])
			return i;
		      ...
		      const unsigned int BufSize, const unsigned int MVA, struct sg_table *sg_table)
{
	...
	if (BufAddr) {
		...
		if (BufAddr < PAGE_OFFSET) {
			...
			M4UTRACE();
			if (vma == NULL || vma->vm_start > BufAddr) {
...
static long MTK_M4U_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case MTK_M4U_T_CACHE_SYNC:
		...
		if (m4u_get_larbid(m4u_cache_data.port) < 0) {
			M4UMSG
			    ("invalid port: MTK_M4U_T_CACHE_SYNC eCacheSync:%d, module:%d, mva:0x%x, va:0x%lx, size:0x%x\n",
			     m4u_cache_data.eCacheSync, m4u_cache_data.port, m4u_cache_data.mva,
			     m4u_cache_data.va, m4u_cache_data.size);
			return -EINVAL;
		}
		pMvaInfo = m4u_client_find_buf(client, m4u_cache_data.mva, 0);
		if (pMvaInfo == NULL ||
		    !(m4u_cache_data.va >= pMvaInfo->va &&
		    m4u_cache_data.va < pMvaInfo->va + m4u_cache_data.size &&
		    m4u_cache_data.size <= pMvaInfo->va + pMvaInfo->size - m4u_cache_data.va)) {
			M4UMSG
			    ("invalid param: MTK_M4U_T_CACHE_SYNC eCacheSync:%d, module:%d, mva:0x%x, va:0x%lx, size:0x%x\n",
			     m4u_cache_data.eCacheSync, m4u_cache_data.port, m4u_cache_data.mva,
			     m4u_cache_data.va, m4u_cache_data.size);
			return -EINVAL;
		}
