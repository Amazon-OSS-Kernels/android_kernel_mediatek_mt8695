...
static long vcodec_alloc_non_cache_buffer(unsigned long arg)
{
	...
	if (rTempMem.eMemCodec != VAL_MEM_CODEC_FOR_VDEC || rTempMem.eMemType != VAL_MEM_TYPE_FOR_HW_NONCACHEABLE) {
		MODULE_MFV_LOGE("[ERROR] VCODEC_ALLOC_NON_CACHE_BUFFER, wrong param!\n");
		return -EFAULT;
	}
...
static long vcodec_free_non_cache_buffer(unsigned long arg)
{
	...
	if (rTempMem.eMemCodec != VAL_MEM_CODEC_FOR_VDEC || rTempMem.eMemType != VAL_MEM_TYPE_FOR_HW_NONCACHEABLE) {
		MODULE_MFV_LOGE("[ERROR] VCODEC_FREE_NON_CACHE_BUFFER, wrong param!\n");
		return -EFAULT;
	}
...
static int vcodec_release(struct inode *inode, struct file *file)
{
	...
	mutex_lock(&DriverOpenCountLock);
	MODULE_MFV_LOGE("vcodec_release pid = %d, MT8695Driver_Open_Count %d\n", current->pid,
		MT8695Driver_Open_Count);
