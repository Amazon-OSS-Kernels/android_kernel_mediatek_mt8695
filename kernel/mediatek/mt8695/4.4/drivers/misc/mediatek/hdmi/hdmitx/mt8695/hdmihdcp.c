...
void vCompareSRM(void)
{
	...
	dwVRLIndex = 0;
	ptrSRM = &_bSRMBuff[8];
	while ((_rSRMInfo.dwVRLLenInDram > dwVRLIndex) && (_rSRMInfo.dwVRLLenInDram > 0) && (_rSRMInfo.dwVRLLenInDram <= MAX_VRL_SIZE)) {
...
{
	unsigned char bTemp, bIndex, bDevice_Count = 0;	/* , bBlock; */
...
{
	unsigned char bIndx, bTemp = 0, BStatus[2] = {0};
...
void vHdcpSrmSetFunc(unsigned int u4ByteCount, unsigned char *prData)
{
	...
	if((_rSRMInfo.dwVRLLenInDram > 0) && (_rSRMInfo.dwVRLLenInDram <= MAX_VRL_SIZE)) {
		...
		HDMI_HDCP_LOG("Err: wrong size of VRLs\n");
		WARN_ON(1);
	}
}
