...
{
	if (_stAvdAVInfo.e_resolution < HDMI_VIDEO_RESOLUTION_NUM)
		HDMI_PLUG_LOG("HDMI output resolution = %s\n", szHdmiResStr[_stAvdAVInfo.e_resolution]);	/*  */
	else
		HDMI_PLUG_LOG("HDMI output resolution out of range.\n");
...
void vSetHDMITxPLL(unsigned int bResIndex, unsigned int bClockRate)
{
	...
	if ((u4Feq == 3) && (bClockRate != TMDS_CLK_X1)) { /*3G~6G*/
		...
		if ((bResIndex == HDMI_VIDEO_1920x1080p_60Hz ||
			...
			HDMI_PLUG_LOG("[hdmi]1080p60/50 12 bits mode\n");
...
VID_PLA_DR_TYPE_T hdr_status(char *str)
{
	...
	case VID_PLA_DR_TYPE_HDR10_PLUS_VSIF:
		...
		if ( hdr10p_vsif_application_version == 0x0)
			sprintf(str, "%s", "HDR10Plus-V0-VSIF");
		else if (hdr10p_vsif_application_version == 0x1)
			sprintf(str, "%s", "HDR10Plus-V1-VSIF");
		else
			sprintf(str, "%s", "HDR10Plus-VSIF with wrong application version");
		break;
	default:
...
{
	if (_stAvdAVInfo.e_resolution < HDMI_VIDEO_RESOLUTION_NUM &&
		_stAvdAVInfo.e_deep_color_bit < HDMI_DEEP_COLOR_T_NUM &&
		_stAvdAVInfo.e_video_color_space < HDMI_OUT_COLOR_SPACE_T_NUM) {
	sprintf(str, "%s\n%s\n%s\n\n",
