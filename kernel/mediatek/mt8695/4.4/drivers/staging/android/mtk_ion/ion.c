...
static struct ion_handle *ion_handle_get_by_id_nolock(struct ion_client *client, int id)
{
	struct ion_handle *handle;
...
static void *ion_buffer_kmap_get(struct ion_buffer *buffer)
{
	...
	if (WARN_ONCE(!vaddr, "heap->ops->map_kernel should return ERR_PTR on error"))
		return ERR_PTR(-EINVAL);
	if (IS_ERR(vaddr))
		return vaddr;
	buffer->vaddr = vaddr;
...
static int ion_client_validate(struct ion_device *dev, struct ion_client *client)
{
	struct rb_node *n;
...
static int ion_debug_client_show(struct seq_file *s, void *unused)
{
	...
	struct ion_device *dev = IS_ENABLED(CONFIG_MTK_ION) ? g_ion_device : NULL;
	struct rb_node *n;
	size_t *sizes;
	const char **names;
	int i;
...
*/
		mutex_lock(&client->lock);
/*
		IONDBG("warn destroy: hdl=%p, buf=%p, ref=%d, sz=%zu, kmp=%d, client %s, disp %s, dbg %s, sec 0x%lx\n",
...
static int ion_sync_for_device(struct ion_client *client, int fd)
{
	struct dma_buf *dmabuf;
	struct ion_buffer *buffer;
	struct device *dev = IS_ENABLED(CONFIG_MTK_ION) ? g_ion_device->dev.this_device : NULL;
...
static int ion_debug_heap_show(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "%16.s %16zu\n", "total ", total_size);
	if (heap->id == ION_HEAP_TYPE_MULTIMEDIA)
		seq_printf(s, "%16.s %16zu\n", "cam-va2mva total", camera_total_size);
	if (heap->flags & ION_HEAP_FLAG_DEFER_FREE)
		seq_printf(s, "%16.s %16zu\n", "deferred free",
			   heap->free_list_size);
	seq_puts(s, "----------------------------------------------------\n");
