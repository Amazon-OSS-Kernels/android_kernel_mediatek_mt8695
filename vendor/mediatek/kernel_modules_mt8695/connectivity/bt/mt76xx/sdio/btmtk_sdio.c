...
static void btmtk_sdio_load_woble_setting_callback(const struct firmware *fw_data, void *context)
{
	...
	if (fw_data) {
		pr_info("%s: woble_setting callback request_firmware size %zu success", __func__, fw_data->size);
		card->woble_setting = kzalloc(fw_data->size + 1, GFP_KERNEL); /* w:move to btmtk_usb_free_memory */
		if (!card->woble_setting) {
			pr_err("%s: kzalloc fail", __func__);
			goto LOAD_END;
		}
	} else {
		pr_err("%s: fw_data is NULL callback request_firmware fail or can't find file!!",
			__func__);
		return;
	}
	if (card->woble_setting == NULL) {
		pr_err("%s: kzalloc size %zu failed!!", __func__, fw_data->size);
		card->woble_setting_len = 0;
		goto LOAD_END;
	}

	memcpy(card->woble_setting, fw_data->data, fw_data->size);
	card->woble_setting[fw_data->size] = '\0';

	card->woble_setting_len = fw_data->size;

	err = btmtk_sdio_load_woble_block_setting("APCF",
			card->woble_setting_apcf, WOBLE_SETTING_COUNT, card->woble_setting);
	if (err)
		goto LOAD_END;

	err = btmtk_sdio_load_woble_block_setting("APCF_ADD_MAC",
			card->woble_setting_apcf_fill_mac, WOBLE_SETTING_COUNT, card->woble_setting);
	if (err)
		goto LOAD_END;

	err = btmtk_sdio_load_woble_block_setting("APCF_ADD_MAC_LOCATION",
			card->woble_setting_apcf_fill_mac_location, WOBLE_SETTING_COUNT, card->woble_setting);
	if (err)
		goto LOAD_END;

	err = btmtk_sdio_load_woble_block_setting("RADIOOFF",
			card->woble_setting_radio_off, WOBLE_SETTING_COUNT, card->woble_setting);
	if (err)
		goto LOAD_END;

	err = btmtk_sdio_load_woble_block_setting("RADIOOFF_STATUS_EVENT",
			card->woble_setting_radio_off_status_event, WOBLE_SETTING_COUNT, card->woble_setting);
	if (err)
		goto LOAD_END;

	err = btmtk_sdio_load_woble_block_setting("RADIOOFF_COMPLETE_EVENT",
			card->woble_setting_radio_off_comp_event, WOBLE_SETTING_COUNT, card->woble_setting);
	if (err)
		goto LOAD_END;

	err = btmtk_sdio_load_woble_block_setting("RADIOON",
			card->woble_setting_radio_on, WOBLE_SETTING_COUNT, card->woble_setting);
	if (err)
		goto LOAD_END;

	err = btmtk_sdio_load_woble_block_setting("RADIOON_STATUS_EVENT",
			card->woble_setting_radio_on_status_event, WOBLE_SETTING_COUNT, card->woble_setting);
	if (err)
		goto LOAD_END;

	err = btmtk_sdio_load_woble_block_setting("RADIOON_COMPLETE_EVENT",
			card->woble_setting_radio_on_comp_event, WOBLE_SETTING_COUNT, card->woble_setting);
	if (err)
		goto LOAD_END;

	err = btmtk_sdio_load_woble_block_setting("APCF_RESMUE",
		card->woble_setting_apcf_resume, WOBLE_SETTING_COUNT, card->woble_setting);
	if (err)
		goto LOAD_END;

	err = btmtk_sdio_load_woble_block_setting("APCF_COMPLETE_EVENT",
			card->woble_setting_apcf_resume_event, WOBLE_SETTING_COUNT, card->woble_setting);

LOAD_END:
	release_firmware(fw_data);
	if (err)
		pr_warn("%s: result fail", __func__);
	else
		pr_info("%s: result success", __func__);
}
					 ...
					 struct device *dev, u32 *code_len, struct btmtk_sdio_card *data)
{
	...
	if (err < 0) {
		pr_notice("%s: request %s file fail(%d)\n",
...
static void btmtk_set_pa(uint8_t pa)
{
	...
	uint8_t epa_e[] = {0x04, 0x0E, 0x04, 0x01,
		0x70, 0xFD, 0x00};

	epa[4] = pa;
	if (pa > 1 || pa < 0) {
		pr_warn("%s: Incorrect format", __func__);
		return;
	}
	if (pa == 1) {
	...
	ret = btmtk_sdio_send_and_check(epa, sizeof(epa),
		epa_e, sizeof(epa_e));
...
{
	int ret = -1;
	uint8_t ant[] = {0x01, 0x71, 0xFD, 0x01, 0};
	uint8_t ant_e[] = {0x04, 0x0E, 0x04, 0x01,
		0x71, 0xFD, 0x00};
	...
	ret = btmtk_sdio_send_and_check(ant, sizeof(ant),
		ant_e, sizeof(ant_e));
...
static void btmtk_set_pa_and_duplex(uint8_t *buf, size_t buf_size)
{
	...
	if (*ptr != '0') {
		pr_warn("%s: ePA mode: %c\n", __func__, *ptr);
		g_card->pa_setting = 1;
	} else {
		pr_warn("%s: iPA mode: %c\n", __func__, *ptr);
		g_card->pa_setting = 0;
	}
	...
	if (p > p_buf) {
		...
		while ((*p == ' ') && (p != p_buf))
			p--;
		if (*p == '#') {
			pr_warn("%s: It's no duplex setting\n", __func__);
			g_card->duplex_setting = -1;
			goto out;
		}
	}
	...
	if (*ptr != '0') {
		pr_warn("%s: TDD mode: %c\n", __func__, *ptr);
		g_card->duplex_setting = 1;
	} else {
		pr_warn("%s: FDD mode: %c\n", __func__, *ptr);
						...
						bool is7668)
{
	...
	if (!buf) {
		...
		pr_warn("%s: only support mt7668 right now\n",
			__func__);
		return;
	}
	...
	if (is7668) {
		...
		/* TX MAX power */
		set_radio[9] = (*(buf + offset + 2));
		/* TX power sub level */
		set_radio[10] = 0;
		/* BR/EDR power diff mode */
		set_radio[11] = (*(buf + offset + 3));
	}
	...
	if ((buf[0] == 0xFF && buf[2] == 0x50) ||
		...
		if (skb_queue_len(&g_priv->adapter->fwlog_fops_queue) < FWLOG_QUEUE_COUNT) {
			...
			if (fwlog_picus_blocking_warn == 0) {
				...
				pr_warn("%s fwlog queue size is full(picus)\n", __func__);
			}
		}
		...
		if (skb_queue_len(&g_priv->adapter->fwlog_fops_queue) < FWLOG_ASSERT_QUEUE_COUNT) {
			...
			if (fwlog_fwdump_blocking_warn == 0) {
				...
				pr_warn("%s fwlog queue size is full(coredump)\n", __func__);
			}
		}
	}
	return ret;
...
static int btmtk_sdio_read_pin_mux_setting(u32 *value)
{
	...
	pr_debug("%s rx_length %d\n", __func__, rx_length);
	pr_debug("%s rx_length %d %02x%02x%02x%02x%02x\%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\n",
...
static int btmtk_sdio_download_rom_patch(struct btmtk_sdio_card *card)
{
	...
	chip_id = btmtk_sdio_bt_memRegister_read(CHIP_ID);
	...
	if (rx_length > (SDIO_HEADER_LEN + 8) && rxbuf[SDIO_HEADER_LEN] == 0x80 &&
...
#endif/*end SAVE_FW_DUMP_IN_KERNEL*/
		}
		...
		if (skb_queue_len(&g_priv->adapter->fwlog_fops_queue) < FWLOG_ASSERT_QUEUE_COUNT) {
			...
			pr_debug("%s : Receive coredump data, move data to fwlog queue for picus",
				__func__);
			buf_len = rx_length - (MTK_SDIO_PACKET_HEADER_SIZE + 1);
			btmtk_sdio_dispatch_fwlog(&rxbuf[MTK_SDIO_PACKET_HEADER_SIZE + 1], dump_len + 3);
			fwdump_blocking_warn = 0;
		} else if (fwdump_blocking_warn == 0) {
			fwdump_blocking_warn = 1;
			pr_warn("btmtk_usb FW dump queue size is full");
		}
...
SKIP_DUMP:
	...
	if (rx_length > (SDIO_HEADER_LEN + 4) &&
		((rxbuf[SDIO_HEADER_LEN] == 0x04 && rxbuf[SDIO_HEADER_LEN + 1] == 0xFF && rxbuf[SDIO_HEADER_LEN + 3] == 0x50) ||
		(rxbuf[SDIO_HEADER_LEN] == 0x02 && rxbuf[SDIO_HEADER_LEN + 1] == 0xFF && rxbuf[SDIO_HEADER_LEN + 2] == 0x05))) {
	...
	if ((!fw_is_doing_coredump) &&
		...
		if (type == 0x80) {/*this may be coredump data*/
			if (buf_len > 7) {
...
static int btmtk_sdio_reset_dev(struct btmtk_sdio_card *card)
{
	...
	if (ret) {
		pr_err("%s read SDIO_CCCR_IENx %x, func num %d, ret %d\n",
	...
	if (ret) {
		pr_err("%s f0_writeb SDIO_CCCR_IENx %x, func num %d, ret %d error\n",
			__func__, reg, func->num, ret);
		goto reset_dev_end;
	}
	...
	reg = sdio_f0_readb(func, SDIO_CCCR_IENx, &ret);
	if (ret) {
		pr_err("%s f0_readb SDIO_CCCR_IENx %x, func num %d, ret %d error\n",
			__func__, reg, func->num, ret);
		goto reset_dev_end;
	}
	...
	ret = sdio_set_block_size(card->func, SDIO_BLOCK_SIZE);
	if (ret) {
		pr_err("%s cannot set SDIO block size\n", __func__);
		goto reset_dev_end;
	}

	reg = sdio_readb(func, card->reg->io_port_0, &ret);
	if (ret < 0) {
		pr_err("%s read io port0 fail\n", __func__);
		goto reset_dev_end;
	}
	...
	if (card->reg->int_read_to_clear) {
		...
		if (ret < 0) {
			pr_err("%s read init rsr fail\n", __func__);
			goto reset_dev_end;
		}
		sdio_writeb(func, reg | 0x3f, card->reg->host_int_rsr, &ret);
		if (ret < 0) {
			pr_err("%s write init rsr fail\n", __func__);
			goto reset_dev_end;
		}

		reg = sdio_readb(func, card->reg->card_misc_cfg, &ret);
		if (ret < 0) {
			pr_err("%s read misc cfg fail\n", __func__);
			goto reset_dev_end;
		}
...
static int btmtk_sdio_reset_fw(struct btmtk_sdio_card *card)
{
	...
	if (ret) {
		pr_err("%s btmtk_sdio_reset_dev failed!\n", __func__);
		return ret;
	}
...
#if SUPPORT_ANDROID
	pr_info("%s SUPPORT_ANDROID is define 1\n", __func__);
#else
	pr_info("%s SUPPORT_ANDROID is not define 1\n", __func__);
#endif

#if SUPPORT_UNIFY_WOBLE
	pr_info("%s SUPPORT_UNIFY_WOBLE is define 1\n", __func__);
#else
	pr_info("%s SUPPORT_UNIFY_WOBLE is not define 1\n", __func__);
#endif

#if SUPPORT_EINT
	pr_info("%s SUPPORT_EINT is define 1\n", __func__);
#else
	pr_info("%s SUPPORT_EINT is not define 1\n", __func__);
#endif
...
static int btmtk_sdio_suspend(struct device *dev)
{
	...
	if (func) {
		...
		if (ret) {
			pr_err("%s: set flag %d err %d\n", __func__, pm_flags, ret);
			return -ENOSYS;
		}

		pm_flags = sdio_get_host_pm_caps(func);
		pr_debug("%s:%s suspend: PM flags = 0x%x\n",
			__func__, sdio_func_id(func), pm_flags);
		if (!(pm_flags & MMC_PM_KEEP_POWER)) {
			pr_err("%s:%s cannot remain alive while suspended\n",
...
static int btmtk_fops_open(struct inode *inode, struct file *file)
{
	...
	pr_err("%s: Mediatek Bluetooth SDIO driver ver %s", __func__, VERSION);
	pr_err("%s: major %d minor %d (pid %d), probe counter: %d",
			...
			loff_t *f_pos)
{
	...
	if (skb->len <= count) {
		...
		pr_err("%s: socket buffer length error(count: %d, skb.len: %d)", __func__, (int)count, skb->len);
	}
	kfree_skb(skb);
OUT:
	return copyLen;
...
#if SUPPORT_EINT
		if (g_card)
			wake_lock_destroy(&g_card->eint_wlock);
		else
			pr_err("%s:g_data is NULL, no destroy eint_wlock", __func__);
#endif
