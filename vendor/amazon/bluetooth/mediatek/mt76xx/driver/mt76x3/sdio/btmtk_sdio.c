...
int btmtk_sdio_notify_wlan_remove_start(void)
{
	...
	if (pnotify_wlan_remove_start_func) {
		...
		__symbol_put(notify_wlan_remove_start_func_name);
...
static void btmtk_sdio_initialize_cfg_items(void)
{
	...
	g_card->bt_cfg.support_send_vsc_cmd = 0;
	btmtk_sdio_free_fw_cfg_struct(&g_card->bt_cfg.picus_filter, 1);
	btmtk_sdio_free_fw_cfg_struct(g_card->bt_cfg.wmt_cmd, WMT_CMD_COUNT);
	btmtk_sdio_free_fw_cfg_struct(g_card->bt_cfg.vendor_cmd, VENDOR_CMD_COUNT);
					...
					char *searchcontent)
{
	...
	ret = btmtk_sdio_parse_bt_cfg_file(BT_WOBT_BY_SDIO_EINT, text, searchcontent);
	if (ret) {
		btmtk_sdio_bt_cfg_item_value_to_bool(text, &bt_cfg_content->support_wobt_by_sdio);
		BTMTK_INFO("bt_cfg_content->support_wobt_by_sdio = %d",
					bt_cfg_content->support_wobt_by_sdio);
	} else {
		BTMTK_WARN("search item %s is invalid!", BT_WOBT_BY_SDIO_EINT);
	}
	...
	ret = btmtk_sdio_parse_bt_cfg_file(BT_SEND_VSC_CMD, text, searchcontent);
	if (ret) {
		btmtk_sdio_bt_cfg_item_value_to_bool(text, &bt_cfg_content->support_send_vsc_cmd);
		BTMTK_INFO("bt_cfg_content->support_send_vsc_cmd = %d",
				bt_cfg_content->support_send_vsc_cmd);
	} else {
		BTMTK_WARN("search item %s is invalid!", BT_SEND_VSC_CMD);
	}
...
static inline void btmtk_sdio_wobt_wake_lock(struct btmtk_sdio_card *data)
{
	g_card->wake_lock_attempt_count++;
	__pm_stay_awake(data->wobt_ws);
}

static inline void btmtk_sdio_wobt_wake_unlock(struct btmtk_sdio_card *data)
{
	BTMTK_INFO("%s: wake lock attempts: %u", __func__, g_card->wake_lock_attempt_count);
	g_card->wake_lock_attempt_count = 0;
	__pm_relax(data->wobt_ws);
}

#if (KERNEL_VERSION(4,15,0) > LINUX_VERSION_CODE)
void btmtk_sdio_wobt_wake_timeout(unsigned long data)
#else
void btmtk_sdio_wobt_wake_timeout(struct timer_list *t)
#endif
{
	btmtk_sdio_wobt_wake_unlock(g_card);
}
...
static int btmtk_sdio_bt_set_LPReq(void)
{
	int ret = 0;
	...
	ret = btmtk_sdio_send_hci_cmd(HCI_COMMAND_PKT,
		...
		event, sizeof(event), COMP_EVENT_TIMO);
	if (ret != 0) {
		BTMTK_ERR("btmtk_sdio_bt_set_LPReq fail,ret = %d\n", ret);
	} else {
		BTMTK_INFO("end\n");
	}
...
static int btmtk_sdio_send_vendor_cfg(void)
{
	int ret = 0;
	int index = 0;
	uint8_t event[EVT_CMPL_LEN] = {0x0E};
	...
	for (index = 0; index < VENDOR_CMD_COUNT; index++) {
		if (g_card->bt_cfg.vendor_cmd[index].content && g_card->bt_cfg.vendor_cmd[index].length) {
			ret = btmtk_sdio_send_hci_cmd(HCI_COMMAND_PKT,
					...
					event, EVT_CMPL_LEN, COMP_EVENT_TIMO);
			if (ret) {
...
#if SUPPORT_MT7663
	if (is_mt7663(g_card))
		sprintf(fw_version_str, "20%.2s-%.2s-%.2s.%.2s:%.2s.%.7s",
			fw_firmware->data, fw_firmware->data + 2, fw_firmware->data + 4,
			fw_firmware->data + 6, fw_firmware->data + 8,
			fw_firmware->data + 16);
	else
#endif
	{
...
static int btmtk_sdio_enable_host_int(struct btmtk_sdio_card *card)
{
	...
	if (0) {
		...
		if (func_sdio_hook) {
			...
			__symbol_put("mmc_set_bus_width");
		}
		data = sdio_f0_readb(card->func, SDIO_CCCR_IF, &ret);
		if (ret)
...
static int btmtk_sdio_set_card_clkpd(int on)
{
	...
	if (psdio_set_card_clkpd_func) {
		...
		__symbol_put(sdio_set_card_clkpd_func_name);
		ret = 0;
	} else
...
int btmtk_sdio_bt_trigger_core_dump(int trigger_dump)
{
	...
	/* forcing toggle reset without coredump
	 * no matter power_on/fops_open
	 */
...
void btmtk_sdio_notify_wlan_toggle_rst_end(void)
{
	...
	if (pnotify_wlan_toggle_rst_end_func) {
		...
		__symbol_put(notify_wlan_toggle_rst_end_func_name);
	} else
		BTMTK_ERR("do not get %s",
			notify_wlan_toggle_rst_end_func_name);
}
...
int WF_rst_L0_notify_BT_step2(void)
{
	...
	if (is_mt7663(g_card)) {
		...
		ret = btmtk_sdio_notify_wlan_remove_end();
...
int WF_rst_L0_notify_BT_step1(int force_toggle_reset)
{
	...
	/* if BT is not ready for reset, return a non-positive value */
	if (force_toggle_reset == true) {
		if (is_mt7663(g_card)) {
			BTMTK_INFO(L0_RESET_TAG "forcing reset dongle begin");
			ret = btmtk_sdio_bt_trigger_core_dump(false);
			if (ret > 0)
				BTMTK_INFO(L0_RESET_TAG "done");
			else
				BTMTK_INFO(L0_RESET_TAG "failed");
		} else {
			BTMTK_ERR(L0_RESET_TAG "is not MT7663");
		}
	} else {
		if (is_mt7663(g_card)) {
			BTMTK_INFO(L0_RESET_TAG "begin");
			ret = btmtk_sdio_bt_trigger_core_dump(true);
			if (ret > 0)
				BTMTK_INFO(L0_RESET_TAG "done");
			else
				BTMTK_INFO(L0_RESET_TAG "failed");
...
#ifdef abc123_AML_TEMP_CHIP_RESET_FIX
extern void set_usb_wifi_power(int a);
#endif
static int btmtk_sdio_L0_reset(struct mmc_card *card)
{
	...
	psdio_mmc_start_host psdio_mmc_start_host_func =
		(psdio_mmc_start_host) btmtk_kallsyms_lookup_name("mmc_start_host");
	psdio_mmc_stop_host psdio_mmc_stop_host_func =
		(psdio_mmc_stop_host) btmtk_kallsyms_lookup_name("mmc_stop_host");
	...
	if (psdio_mmc_start_host_func && psdio_mmc_stop_host_func) {
		BTMTK_INFO(L0_RESET_TAG "mmc_stop_host");
...
#ifdef abc123_AML_TEMP_CHIP_RESET_FIX
	BTMTK_ERR(L0_RESET_TAG "set_usb_wifi_power 0");
	set_usb_wifi_power(0);
	msleep(200);
	BTMTK_ERR(L0_RESET_TAG "set_usb_wifi_power 1");
	set_usb_wifi_power(1);
#endif

	if (psdio_mmc_start_host_func && psdio_mmc_stop_host_func) {
		BTMTK_INFO(L0_RESET_TAG "mmc_start_host");
		psdio_mmc_start_host_func(host);
		BTMTK_INFO(L0_RESET_TAG "mmc_start_host done");
					...
					const struct sdio_device_id *id)
{
...
#if (defined(LINUX_OS) && (KERNEL_VERSION(5, 4, 0) > LINUX_VERSION_CODE)) ||	\
	(defined(ANDROID_OS) && (KERNEL_VERSION(4, 19, 0) > LINUX_VERSION_CODE))
		g_card->wobt_ws = wakeup_source_register("btevent_wobt");
#else
		g_card->wobt_ws = wakeup_source_register(NULL, "btevent_wobt");
#endif
		if (!g_card->wobt_ws) {
			if (g_card->woble_ws)
...
#else
		timer_setup(&g_card->wake_lock_timer, btmtk_sdio_wobt_wake_timeout, 0);
#endif
		g_card->wake_lock_attempt_count = 0;
	}
	...
	(defined(ANDROID_OS) && (KERNEL_VERSION(4, 19, 0) > LINUX_VERSION_CODE))
		...
		if (!g_card->eint_ws) {
			if (g_card->wobt_ws)
				wakeup_source_unregister(g_card->wobt_ws);
...
static void btmtk_sdio_remove(struct sdio_func *func)
{
	...
	probe_ready = false;
	g_card->dongle_state = BT_SDIO_DONGLE_STATE_UNKNOWN;
	...
	if (func) {
		...
		if (card) {
			...
			if (g_card->bt_cfg.support_wobt_by_sdio) {
				btmtk_sdio_wobt_wake_unlock(g_card);
				if (card->wobt_ws)
					wakeup_source_unregister(card->wobt_ws);
				del_timer(&g_card->wake_lock_timer);
				g_card->wake_lock_attempt_count = 0;
			}
...
static int btmtk_sdio_send_apcf_reserved(void)
{
	...
	/*index number adjust from 12 to 11 (because FW modify code more easy if total index is 11),*/
	/*10 is for app,1 for woble */
...
static int btmtk_fops_open(struct inode *inode, struct file *file)
{
	...
	if (g_priv->btmtk_dev.reset_progress || g_priv->btmtk_dev.reset_dongle) {
			BTMTK_ERR("%s reset_progress is %d, reset_dongle is %d", __func__,
				g_priv->btmtk_dev.reset_progress, g_priv->btmtk_dev.reset_dongle);
		return -EAGAIN;
	}

	if (g_card->dongle_state == BT_SDIO_DONGLE_STATE_FW_DUMP) {
	...
	if (fops_state == BTMTK_FOPS_STATE_OPENING) {
		BTMTK_ERR("mode is %d", fops_state);
		FOPS_MUTEX_UNLOCK();
		return -ENOENT;
	}
			...
			size_t count, loff_t *f_pos)
{
	...
	if (!probe_ready) {
		BTMTK_WARN_LIMITTED("probe_ready is %d return\n", probe_ready);
		return -EFAULT;
	}
	...
	if (get_hci_reset == 1) {
		...
		if (g_card->bt_cfg.support_send_vsc_cmd == true)
			btmtk_sdio_bt_set_LPReq();
			...
			size_t count, loff_t *f_pos)
{
	...
	if (o_fwlog_buf[0] == 0x01 && o_fwlog_buf[1] == 0xBE && o_fwlog_buf[2] == 0xFC) {
		if (o_fwlog_buf[4] == 0x15) {
			g_card->bt_cfg.support_auto_picus = true;
			BTMTK_DBG("auto_picus TRUE");
		} else if (o_fwlog_buf[4] == 0x00) {
			g_card->bt_cfg.support_auto_picus = false;
			BTMTK_DBG("auto_picus FALSE");
		}
	}
