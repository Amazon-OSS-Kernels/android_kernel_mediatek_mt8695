...
#if CFG_SHOW_HIDDEN_NETWORK
	if (fgIsProbeResp || fgIsValidSsid || !prBssDesc->u2RawLength)
#else
	if (fgIsProbeResp || fgIsValidSsid)
#endif
	{
	...
	const uint16_t logBufLen)
{
	...
	idx += kalScnprintf(logBuf, sizeof(logBuf), "%u: ", prList->u4NumElem);
...
#if CFG_SHOW_FULL_MACADDR
		idx += kalScnprintf(logBuf+idx, dataLen+1,
