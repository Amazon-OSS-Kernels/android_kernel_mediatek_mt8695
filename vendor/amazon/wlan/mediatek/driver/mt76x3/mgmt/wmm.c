...
void wmmComposeTsmRpt(struct ADAPTER *prAdapter,
	...
	uint32_t u4EventBufLen)
{
	...
	if (u4EventBufLen < sizeof(struct CMD_GET_TSM_STATISTICS))
	{
		DBGLOG(RLM, ERROR,
			"%s(): Invalid length:%d < %d\n",
			__func__, u4EventBufLen, sizeof(struct CMD_GET_TSM_STATISTICS));
		return;
	}
